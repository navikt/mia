...
export function getParamsForValgteFylkerOgKommuner(state) {
  ...
  const harIkkeValgtKommuneIFylke = valgtFylke => !state.rest.omrader.data.find(fylke => fylke.id === valgtFylke).underomrader.find(kommune => valgteKommuner.includes(kommune.id));
